...
/// Pin an agent as context for the current Athena session.
#[tauri::command]
pub async fn athena_pin_agent(
    ...
    orchestrator.pin_context(athena_core::orchestrator::PinnedContext::Agent {
        pane_id,
        agent_type,
        status,
    });
...
/// Pin a Kanban task as context for the current session.
#[tauri::command]
pub async fn athena_pin_task(
    state: State<'_, AppState>,
    task_id: String,
    title: String,
    status: String,
    ...
    let orchestrator = Arc::clone(&state.orchestrator);
    orchestrator.pin_context(athena_core::orchestrator::PinnedContext::KanbanTask {
        task_id,
        title,
        status,
    });
...
/// Pin a file as context for the current session.
#[tauri::command]
pub async fn athena_pin_file(
    state: State<'_, AppState>,
    path: String,
    preview_lines: Vec<String>,
) -> Result<(), String> {
    let orchestrator = Arc::clone(&state.orchestrator);
    orchestrator.pin_context(athena_core::orchestrator::PinnedContext::File {
        path,
        preview_lines,
    });
    Ok(())
...
#[tauri::command]
pub async fn athena_clear_pinned_context(state: State<'_, AppState>) -> Result<(), String> {
    let orchestrator = Arc::clone(&state.orchestrator);
    orchestrator.clear_pinned_context();
    Ok(())
}
...
pub async fn athena_get_pinned_context(state: State<'_, AppState>) -> Result<Vec<serde_json::Value>, String> {
    ...
    Ok(orchestrator
        ...
        .into_iter()
        .map(|item| match item {
            athena_core::orchestrator::PinnedContext::Agent { pane_id, agent_type, status } => {
                serde_json::json!({"type": "agent", "pane_id": pane_id, "agent_type": agent_type, "status": status})
            }
            athena_core::orchestrator::PinnedContext::KanbanTask { task_id, title, status } => {
                serde_json::json!({"type": "kanban_task", "task_id": task_id, "title": title, "status": status})
            }
            athena_core::orchestrator::PinnedContext::File { path, preview_lines } => {
                serde_json::json!({"type": "file", "path": path, "preview_lines": preview_lines})
            }
        })
        .collect())
}
