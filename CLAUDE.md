...
- **Known WASM runtime issue (mitigated)**: Dioxus 0.7 event handlers can cause `RuntimeError: Unreachable code should not be executed` panics in WKWebView after clicks. This is mitigated three ways and is not a launch blocker: (1) Dioxus ≥0.7.6 catches event-handler panics so they no longer brick the page (the workspace pins 0.7.9); (2) `panic = "unwind"` (set for both dev and release) prevents RefCell poisoning after a panic; (3) the JS watchdog in `index.html` reloads on heartbeat silence. Remaining E2E flakiness (`tauri-wd` sessions dying ~1s after creation) is a separate WebDriver-infra issue. The only outstanding verification is the 4–8h packaged soak in `docs/release/MACOS_TEST_MATRIX.md`.
