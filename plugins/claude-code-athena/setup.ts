...
async function main() {
  ...
  if (command === 'discover') {
    const result = discoverClaudeCode(projectRoot)
    console.log(JSON.stringify(result, null, 2))
    return
  }

  if (command === 'remove') {
    const result = removeMcpEntry('claude-code', useGlobal ? undefined : projectRoot)
    console.log(JSON.stringify(result, null, 2))
    return
  }
  ...
  if (command === 'check') {
    ...
    const discovery = discoverClaudeCode(projectRoot)
  ...
  if (command === 'setup' || command === 'install') {
    ...
    const result = setupClaudeCode(options)

    if (result.success) {
      const verb = result.created ? 'Created' : result.updated ? 'Updated' : 'Configured'
      console.log(`${verb} Claude Code MCP config at: ${result.configPath}`)
      ...
      console.log('\nAthena MCP server is now available in Claude Code as the "athena" MCP server.')
...
Commands:
  setup, install Configure Claude Code to connect to Athena MCP server
  discover       Check if Claude Code is installed and show config status
  remove         Remove Athena MCP entry from Claude Code config
  check          Check if Athena MCP server is reachable
  forward        Start output forwarding (hooks stdout/stderr to Athena)

Options:
  --global Use global config directory (~/.claude/) instead of project
