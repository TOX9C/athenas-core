...
pub fn init<R: Runtime>() -> tauri::plugin::TauriPlugin<R> {
    ...
    tauri::plugin::Builder::new("webdriver-automation")
        ...
        .on_webview_ready(move |webview| {
            // Child webviews (e.g. created via Window::add_child) do not have an
            // associated WebviewWindow, so get_webview_window returns None.
            // Skip them — the WebDriver server only manages top-level windows.
            let Some(window) = webview.get_webview_window(webview.label()) else {
                return;
            };
            let _ = webview_created_tx.send(window);
        })
        .setup(move |app, _api| {
