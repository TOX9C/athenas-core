...
import { InputRequestModal } from './components/Plugin/InputRequestModal'
import { PluginEventBus } from './components/Plugin/PluginEventBus'
import { AgentInspector } from './components/Agents/AgentInspector'
import { OutputEventBus } from './components/Agents/OutputEventBus'
...
export default function App() {
  ...
  useEffect(() => {
    window.athena.store.get('theme').then((saved: any) => {
      if (saved && themes[saved as ThemeName]) {
        useUIStore.getState().setTheme(saved as ThemeName)
        applyTheme(themes[saved as ThemeName])
      }
    })
    ...
    window.athena.store.get('layout').then((saved: any) => {
      if (saved && typeof saved === 'object') {
        useLayoutStore.getState().hydrateFromSaved(saved)
        if (saved.panelSizes?.sidebarWidth)
          useUIStore.getState().setSidebarWidth(saved.panelSizes.sidebarWidth)
        if (saved.activePanel) useUIStore.getState().setActivePanel(saved.activePanel)
        if (saved.activeSidebarSection)
          useUIStore.getState().setSidebarSection(saved.activeSidebarSection)
        if (saved.activeSpaceId) useWorkspaceStore.getState().setActiveSpace(saved.activeSpaceId)
      }
    })
    ...
    window.athena.store.get('athena-lastSessionId').then((saved: any) => {
      if (typeof saved === 'string') {
        useSessionStore
          .getState()
          .switchSession(saved)
          .then(() => {
            const msgs = useSessionStore.getState().activeSessionMessages
            if (msgs.length > 0) {
  ...
  useEffect(() => {
    const unsubClose = window.athena.pty.onAthenaClosePanes((paneIds: string[]) => {
      ...
      window.athena.pty.ackClosePanes({ success: true })
    })
    ...
    const unsub = window.athena.pty.onAthenaSpawn((data: any) => {
      ...
      window.athena.pty.ackAgentSpawned(data.id)
    })
    return () => {
      unsub()
      unsubClose()
  ...
  useEffect(() => {
    ...
    useLayoutStore.getState().setLayout({ activeSpaceId })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [spaces])

  const latestStateRef = useRef({ platform, spaces })

  useEffect(() => {
    latestStateRef.current = { platform, spaces }
  }, [platform, spaces])

  useEffect(() => {
    useLayoutStore.getState().setLayout({ sidebarOpen })
  }, [sidebarOpen])

  useEffect(() => {
    useLayoutStore.getState().setLayout({ activePanel })
  }, [activePanel])

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      const currentPlatform = latestStateRef.current.platform
      const currentSpaces = latestStateRef.current.spaces
      const mod = currentPlatform === 'darwin' ? e.metaKey : e.ctrlKey
      ...
      if (mod && e.key === 'j') {
        e.preventDefault()
        useAthenaStore.getState().toggleOpen()
      }
      ...
      if (mod && e.key >= '1' && e.key <= '9') {
        ...
        if (currentSpaces[idx]) {
          useWorkspaceStore.getState().setActiveSpace(currentSpaces[idx].id)
        }
      }
      if (e.key === 'Escape') {
      ...
      if (mod && e.shiftKey && e.key === 'R') {
        ...
        useUIStore.getState().setActivePanel('terminals')
        useUIStore.getState().setSidebarSection('spaces')
      }
    }
  ...
  return (
    <ErrorBoundary>
      ...
      >
        ...
        >
          ...
          {!isMac && (
            <div className="flex items-center shrink-0 no-drag">
              ...
              <button
                onClick={() => {
                  window.athena.window.maximize()
                  setIsMaximized(!isMaximized)
                }}
                className="h-[38px] w-[46px] flex items-center justify-center hover:bg-white/10 transition-colors"
              >
                {isMaximized ? (
                  <Copy size={12} style={{ color: 'var(--textMuted)' }} />
                ) : (
                  <Square size={12} style={{ color: 'var(--textMuted)' }} />
                )}
              </button>
              <button
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0">
            <div className="flex-1 flex min-h-0">
              <div
                ...
                style={{ flexBasis: browserOpen ? '60%' : '100%' }}
              >
                {renderMainContent()}
              </div>
              {browserOpen && (
        ...
        <PluginEventBus />
        <OutputEventBus />
        {/* Invisible triggers for command palette integration */}
        <button data-new-space-trigger onClick={() => setShowNewSpace(true)} className="hidden" />
        <button data-swarm-trigger onClick={() => setShowSwarmModal(true)} className="hidden" />
      </div>
    </ErrorBoundary>
  )
}
...
function SidebarRail({ onExpand }: { onExpand: () => void }) {
  ...
  const handleSectionClick = (section: 'spaces' | 'files' | 'agents' | 'plugins') => {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title="Plugins"
      >
        <Puzzle size={12} style={{ color: 'var(--textDim)' }} />
      </button>
    </div>
  )
}
